            ...
            for row in parsed_rows:
                try:
                    ...
                    for i, (col_name, col_type) in enumerate(column_info):
                        if "UInt" in col_type:  # Check if the column type requires an integer (UInt)
                            try:
                                # Convert to int if it's a float with no fractional part
                                value = row[i] if not isinstance(row[i], float) or row[i].is_integer() else int(row[i])
                                row[i] = value  # Modify the row to hold valid data
                            except ValueError:
                                logging.error(f"Cannot convert '{row[i]}' to {col_type} in column '{col_name}'. Skipping this row.")
                                logging.info(f"Invalid row (skipping): {row}")
                                valid = False
                                break  # Exit validation loop if there's a mismatch
                    if valid:
