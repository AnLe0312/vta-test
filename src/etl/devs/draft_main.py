...
from modules.schema_handler import fetch_table_last_modified, fetch_table_schema, transform_dataframe_to_schema, prepare_sql_data, get_max_modified_datetime_from_schema, update_last_modified_in_schema
...
def extract(logger):
    ...
    table_last_modified = fetch_table_last_modified(database_name, table_name)
    
    # Return the entire file or filter based on table_updated_at
    if table_last_modified is None or df['MODIFIEDDATETIME'].max() >= table_last_modified:
        extracted_data = df[df['MODIFIEDDATETIME'] >= table_last_modified].copy().reset_index(drop=True) if table_last_modified else df
        logger.info(f"Extracted {len(extracted_data)} records from GCS.")
        logger.debug(f"MODIFIEDDATETIME range in extracted data: {extracted_data['MODIFIEDDATETIME'].min()} → {extracted_data['MODIFIEDDATETIME'].max()}")
...
def load(data, logger):
    ...
    new_last_modified = get_max_modified_datetime_from_schema(database_name, table_name)
    update_last_modified_in_schema(database_name, table_name, new_last_modified, logger=logger)
